...
function sendMessage() {
	...
	var displayMessage = "";
	displayMessage = "<tr><td style='text-align: right'><small class='sender-receiver'>[sent to "
			+ $("#receiver").val()
			+ "]	</small><strong>"
			+ $("#messagecontent").val() + "</strong</td></tr>";
...
function showMessages(message) {
	...
	if (loggerUser == message.receiver) {
		displayMessage = "<tr><td style='text-align: left'><small class='sender-receiver'>[sent by "
				+ message.sender
				+ "]	</small><strong>"
				+ message.messageContent + "</strong></td></tr>";
	}
	$("#messages").append(displayMessage);
}
...
function getUsers(){
	$.getJSON("/api/all-users", function(objLst){
		var lstUserNames = [];
		for (var i in objLst){
			lstUserNames.push(objLst[i].userName);
		}
...
$(function() {
	...
	// get all users from server and fill the receiver part
	getUsers();
});
